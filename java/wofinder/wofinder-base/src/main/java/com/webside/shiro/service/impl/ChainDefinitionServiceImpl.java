...
public class ChainDefinitionServiceImpl implements ChainDefinitionService {
	...
	public String initFilterChainDefinitions() {
		...
		for (Map.Entry<String, String> entry : chainMap.entrySet()) {
			...
			chain.append(entry.getKey()).append(" = ").append(entry.getValue()).append(CRLF);
		}
	...
	public Map<String, String> initChainDefinitionsMap() {
		...
		try {
			String shiroConfig = ChainDefinitionServiceImpl.class.getResource("/shiroAuth.properties").toURI().getPath();
