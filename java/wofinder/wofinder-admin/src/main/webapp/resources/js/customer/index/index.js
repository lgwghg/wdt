...
var webside = {
                    ...
                    $(this).bind("click", function() {
                        ...
                        if (sn[sn.length - 1] == '/sirona' || sn[sn.length - 1] == '/druid') {
                            ...
                            $(".page-content").load(sys.rootPath + sn[sn.length - 1],function(data, statusTxt, xhr){
                            	...
                            	{
                            		layer.msg("请求服务器超时,请重新登录或刷新浏览器再试!", {title:"",icon: 0});
                            	}else //statusTxt=="success" || statusTxt=="notmodified"
                            	{
                            		...
                            		if(data.match("^\{(.+:.+,*){1,}\}$"))
					            	{
					            		...
					            		if(data.status == "401")
					            		{
					            			layer.msg(data.message, {title:"",icon: 0});
    ...
    common : {
        ...
        delModel : function(nav, callback) {
            var rows = grid.getCheckedRecords();
            if (rows.length == 1) {
                if (nav == '/userCtrl/deleteBatch.html') {
            ...
            validateUserInfoForm : function() {
                ...
                $('#userInfoForm').validate({
                    ...
                    submitHandler : function(form) {
                        webside.common.commit('userInfoForm', '/userCtrl/edit.html', '/userCtrl/infoUI.html?id=' + $("#id").val());
                    }
                });
            }
        },
            ...
            validateResourceForm : function() {
                $('#resourceForm').validate({
                    ...
                    submitHandler : function(form) {
                        ...
                        webside.common.commit('resourceForm', url, '/resource/listGridUI.html');
                    }
                });
            },
            delModel : function(nav, callback) {
    ...
    wodota : {
         ...
         * @param {Object} nav  提交url
         */
        editModelForModal : function(nav,id,gridId) {
        	if(gridId != undefined && gridId != null){
        		grid = gridId;
        	}
            //当前页码
            ...
            //获取排序方式 [0-不排序，1-正序，2-倒序]
            var sortType = grid.sortParameter.sortType;
            return nav + '?id=' + id + "&page=" + nowPage + "&rows=" + pageSize + "&sidx=" + columnId + "&sord=" + sortType;
        },
        /**
         * 删除(行内)
         * @param {Object} nav  提交url
         ...
         * @param {Object} commitUrl 表单提交地址
         * @param {Object} listUrl 表单提交成功后转向的列表页地址
         * @param {Object} modalId 模型框的id
         * @param {Object} listMethed 回调查询方法名
         * @param {Object} baseListMethed 父级列表回调查询方法名
         */
        commit : function(formId, commitUrl, modalId, listMethed ,baseListMethed) {
            //组装表单数据
            var index;
            var data = $("#" + formId).serialize();
            $.ajax({
        ...
        /**
         * 弹出选择框
         * 适用场景：新增修改时候，一些选项较多的下拉列表可以改成查询页面的弹框
         * @param {Object} modalId 模型框的id
         * @param {Object} element 触发元素
         */
        show(modalId,element){
        	var href = $(element).attr("href");
        	var hidId = $(element).prev().attr("id");
        	...
        	$("#"+modalId).modal({
        		remote: href+"?hidId="+hidId+"&showId="+showId+str
        	});
        },
        /**
         * 选择选择框的值
         * @param {Object} modalId 模型框的id
         * @param {Object} value 隐藏域的值
         * @param {Object} name 显示域的值
         */
        selectForSearchModal(modalId,value,name){
        	$('#' + $("#hidId").val()).val(value);
        	$('#' + $("#showId").val()).val(name);
        	$('#' + modalId).modal('hide');
        },
