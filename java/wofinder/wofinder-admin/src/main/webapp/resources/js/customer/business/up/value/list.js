var dtGridColumns_v = [
	...
	},
	{
	    ...
	    resolution : function(value, record, column, grid, dataNo, columnNo) {
	    	var content = '';
	    	if(record.edit){
	    		var editUrl = webside.wodota.editModelForModal('/upValueCtrl/editUI.html',record.id);
	    	...
	    	if(record.del){
	    		content+= '<button type="button" onclick="webside.wodota.delModelByOperation(\'/upValueCtrl/delete.html\',customSearch_v,\''+record.id+'\')" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>&nbsp;删除</button>';
	    	}
	    	return content;
	    }
	}
...
$(function() {
    ...
    grid_v.parameters['upId'] = $("#upId").val();
    grid_v.load();
