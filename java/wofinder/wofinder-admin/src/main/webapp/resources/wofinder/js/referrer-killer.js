...
var ReferrerKiller = (function () {
	...
	function htmlString(html, iframeAttributes) {
		...
		return '<iframe \
				style="border:0px solid #ff0000;width:100%;height:100%; \
				position:absolute; \
				top:0; \
				left:0;"\
				scrolling="no" \
			...
			<head>\
			<meta charset=\\\'utf-8\\\'>\
			<style>*{margin:0;padding:0;border:0;width:100%;height:100%}</style>\
			</head>' +
	 ...
	 * @return {String} html
	 */
	var imageHtml = PUB.imageHtml = function (url,href, imgAttributesParam) {
		...
		if(trim(href)!=''){
			return htmlString('<a target="_blank" href="'+href+'"><img src="' + escapeDoubleQuotes(url) + '" ' + objectToHtmlAttributes(imgAttributes) + '/></a>');
		}else{
