...
<div class="row" style="margin-top:5px;">
	<div class="col-xs-12">
		...
		<c:if test="${!empty roleEntity}">
		   ...
		   <div class="form-group">
		      <label class="control-label col-sm-1 no-padding-right" for="name">角色类型</label>
		      <div class="col-sm-10">
		      <div class="clearfix">
		         <%-- <input class="form-control" name="name" id="name" type="text" 
		           value="${roleEntity.name }" placeholder="角色名称..."/> --%>
		         <select class="form-control" name="roleType" id="roleType" placeholder="角色类型...">
		         	<option value="0" <c:if test="${not empty roleEntity.roleType && roleEntity.roleType eq 0}">selected</c:if> <c:if test="${empty roleEntity}">selected</c:if>>官方网站组</option>
		         	<option value="1" <c:if test="${not empty roleEntity.roleType && roleEntity.roleType eq 1}">selected</c:if>>普通用户组</option>
		         </select>
		      </div>
		      </div>
		   </div>
...
<div class="center">
	<button id="btnAdd" type="button" onclick="javascript:$('#roleForm').submit();" class="btn btn-success btn-sm">
		...
		<button id="btn" type="button" onclick="webside.common.loadPage('/role/listUI.html<c:if test="${!empty roleEntity}">?page=${page.pageNum }&rows=${page.pageSize }&sidx=${page.orderByColumn }&sord=${page.orderByType }</c:if>')" class="btn btn-primary btn-sm">
